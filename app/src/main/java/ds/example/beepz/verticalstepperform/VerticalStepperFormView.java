package ds.example.beepz.verticalstepperform;
...
import ds.example.beepz.verticalstepperform.Builder;
import ds.example.beepz.verticalstepperform.Step;
import ds.example.beepz.verticalstepperform.StepHelper;
...
public class VerticalStepperFormView extends LinearLayout {
    ...
    private List<ds.example.beepz.verticalstepperform.StepHelper> stepHelpers;
     ...
     */
    public ds.example.beepz.verticalstepperform.Builder setup(StepperFormListener stepperFormListener, Step<?>... steps) {
        return new ds.example.beepz.verticalstepperform.Builder(this, stepperFormListener, steps);
    }
     ...
     * @return An instance of the stepper form builder. Use it to configure and initialize the form.
     */
    public ds.example.beepz.verticalstepperform.Builder setup(StepperFormListener stepperFormListener, List<ds.example.beepz.verticalstepperform.Step<?>> steps) {
        ds.example.beepz.verticalstepperform.Step<?>[] stepsArray = steps.toArray(new ds.example.beepz.verticalstepperform.Step<?>[0]);
    ...
    public boolean markStepAsCompletedOrUncompleted(int stepPosition, boolean useAnimations) {
        if (stepPosition >= 0 && stepPosition < stepHelpers.size()) {
            ds.example.beepz.verticalstepperform.StepHelper stepHelper = stepHelpers.get(stepPosition);
    ...
    public synchronized int getOpenStepPosition() {
        for (int i = 0; i < stepHelpers.size(); i++) {
            ds.example.beepz.verticalstepperform.StepHelper stepHelper = stepHelpers.get(i);
     ...
     */
    public synchronized ds.example.beepz.verticalstepperform.Step<?> getOpenStep() {
        ds.example.beepz.verticalstepperform.StepHelper openStepHelper = getOpenStepHelper();
    ...
    public void scrollToStepIfNecessary(final int stepPosition, final boolean smoothScroll) {
        if (stepPosition >= 0 && stepPosition < stepHelpers.size()) {
            stepsScrollView.post(() -> {
                ds.example.beepz.verticalstepperform.Step<?> stepInstance = stepHelpers.get(stepPosition).getStepInstance();
    ...
    public synchronized void cancelFormCompletionOrCancellationAttempt() {
        ...
        ds.example.beepz.verticalstepperform.StepHelper stepHelper = stepHelpers.get(openedStepPosition);

        if (style.closeLastStepOnCompletion) {
            ds.example.beepz.verticalstepperform.Step<?> step = stepHelper.getStepInstance();
            if (!step.isOpen()) {
     ...
     */
    public boolean addStep(int index, ds.example.beepz.verticalstepperform.Step<?> stepToAdd) {
        ds.example.beepz.verticalstepperform.StepHelper lastStep = stepHelpers.get(stepHelpers.size() - 1);
        ...
        ds.example.beepz.verticalstepperform.StepHelper stepHelper = new ds.example.beepz.verticalstepperform.StepHelper(internalListener, stepToAdd);
        stepHelpers.add(index, stepHelper);
        for (int i = 0; i < stepHelpers.size(); i++) {
            if (i != index) {
                ds.example.beepz.verticalstepperform.StepHelper previouslyExistingStepHelper = stepHelpers.get(i);
     ...
     */
    public int getStepPosition(ds.example.beepz.verticalstepperform.Step<?> step) {
        for (int i = 0; i < stepHelpers.size(); i++) {
    ...
    private void onConstructed(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        inflater.inflate(ernestoyaquello.com.verticalstepperform.R.layout.vertical_stepper_form_layout, this, true);
        ...
        style.stepNextButtonText =
                getResources().getString(ernestoyaquello.com.verticalstepperform.R.string.vertical_stepper_form_continue_button);
        style.lastStepNextButtonText =
                getResources().getString(ernestoyaquello.com.verticalstepperform.R.string.vertical_stepper_form_confirm_button);
        style.lastStepCancelButtonText =
                getResources().getString(ernestoyaquello.com.verticalstepperform.R.string.vertical_stepper_form_cancel_button);
        style.confirmationStepTitle =
                getResources().getString(ernestoyaquello.com.verticalstepperform.R.string.vertical_stepper_form_confirmation_step_title);
        style.confirmationStepSubtitle = "";
        style.leftCircleSizeInPx =
                getResources().getDimensionPixelSize(ernestoyaquello.com.verticalstepperform.R.dimen.vertical_stepper_form_width_circle);
        style.leftCircleTextSizeInPx =
                getResources().getDimensionPixelSize(ernestoyaquello.com.verticalstepperform.R.dimen.vertical_stepper_form_text_size_circle);
        style.stepTitleTextSizeInPx =
                getResources().getDimensionPixelSize(ernestoyaquello.com.verticalstepperform.R.dimen.vertical_stepper_form_text_size_title);
        style.stepSubtitleTextSizeInPx =
                getResources().getDimensionPixelSize(ernestoyaquello.com.verticalstepperform.R.dimen.vertical_stepper_form_text_size_subtitle);
        style.stepErrorMessageTextSizeInPx =
                getResources().getDimensionPixelSize(ernestoyaquello.com.verticalstepperform.R.dimen.vertical_stepper_form_text_size_error_message);
        style.leftVerticalLineThicknessSizeInPx =
                getResources().getDimensionPixelSize(ernestoyaquello.com.verticalstepperform.R.dimen.vertical_stepper_form_width_vertical_line);
        style.marginFromStepNumbersToContentInPx =
                getResources().getDimensionPixelSize(ernestoyaquello.com.verticalstepperform.R.dimen.vertical_stepper_form_space_between_numbers_and_content);
        style.backgroundColorOfDisabledElements =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_disabled_elements);
        style.stepNumberBackgroundColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_circle);
        style.stepNumberCompletedBackgroundColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_circle);
        style.stepNumberErrorBackgroundColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_circle);
        style.nextButtonBackgroundColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_next_button);
        style.nextButtonPressedBackgroundColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_next_button_pressed);
        style.lastStepCancelButtonBackgroundColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_cancel_button);
        style.lastStepCancelButtonPressedBackgroundColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_cancel_button_pressed);
        style.stepNumberTextColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_text_color_circle);
        style.stepTitleTextColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_text_color_title);
        style.stepSubtitleTextColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_text_color_subtitle);
        style.nextButtonTextColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_text_color_next_button);
        style.nextButtonPressedTextColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_text_color_next_button_pressed);
        style.lastStepCancelButtonTextColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_text_color_cancel_button);
        style.lastStepCancelButtonPressedTextColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_text_color_cancel_button_pressed);
        style.errorMessageTextColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_text_color_error_message);
        style.bottomNavigationBackgroundColor =
                ContextCompat.getColor(context, ernestoyaquello.com.verticalstepperform.R.color.vertical_stepper_form_background_color_bottom_navigation);
        ...
        if (attrs != null) {
            vars = context.getTheme().obtainStyledAttributes(
                    attrs,
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView,
                    defStyleAttr,
                    0);

            if (vars.hasValue(ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_next_button_text)) {
                style.stepNextButtonText = vars.getString(
                        ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_next_button_text);
            }
            if (vars.hasValue(ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_last_button_text)) {
                style.lastStepNextButtonText = vars.getString(
                        ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_last_button_text);
            }
            if (vars.hasValue(ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_cancel_button_text)) {
                style.lastStepCancelButtonText = vars.getString(
                        ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_cancel_button_text);
            }
            if (vars.hasValue(ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_confirmation_step_title_text)) {
                style.confirmationStepTitle = vars.getString(
                        ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_confirmation_step_title_text);
            }
            if (vars.hasValue(ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_confirmation_step_subtitle_text)) {
                style.confirmationStepSubtitle = vars.getString(
                        ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_confirmation_step_subtitle_text);
            }
            style.leftCircleSizeInPx = vars.getDimensionPixelSize(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_circle_size,
                    style.leftCircleSizeInPx);
            style.leftCircleTextSizeInPx = vars.getDimensionPixelSize(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_circle_text_size,
                    style.leftCircleTextSizeInPx);
            style.stepTitleTextSizeInPx = vars.getDimensionPixelSize(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_title_text_size,
                    style.stepTitleTextSizeInPx);
            style.stepSubtitleTextSizeInPx = vars.getDimensionPixelSize(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_subtitle_text_size,
                    style.stepSubtitleTextSizeInPx);
            style.stepErrorMessageTextSizeInPx = vars.getDimensionPixelSize(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_error_message_text_size,
                    style.stepErrorMessageTextSizeInPx);
            style.leftVerticalLineThicknessSizeInPx = vars.getDimensionPixelSize(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_vertical_line_width,
                    style.leftVerticalLineThicknessSizeInPx);
            style.marginFromStepNumbersToContentInPx = vars.getDimensionPixelSize(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_horizontal_margin_from_step_numbers_to_content,
                    style.marginFromStepNumbersToContentInPx);
            style.backgroundColorOfDisabledElements = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_disabled_elements_background_color,
                    style.backgroundColorOfDisabledElements);
            style.stepNumberBackgroundColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_circle_background_color,
                    style.stepNumberBackgroundColor);
            style.stepNumberCompletedBackgroundColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_circle_completed_background_color,
                    style.stepNumberCompletedBackgroundColor);
            style.stepNumberErrorBackgroundColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_circle_error_background_color,
                    style.stepNumberErrorBackgroundColor);
            style.nextButtonBackgroundColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_next_button_background_color,
                    style.nextButtonBackgroundColor);
            style.nextButtonPressedBackgroundColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_next_button_pressed_background_color,
                    style.nextButtonPressedBackgroundColor);
            style.lastStepCancelButtonBackgroundColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_cancel_button_background_color,
                    style.lastStepCancelButtonBackgroundColor);
            style.lastStepCancelButtonPressedBackgroundColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_cancel_button_pressed_background_color,
                    style.lastStepCancelButtonPressedBackgroundColor);
            style.stepNumberTextColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_circle_text_color,
                    style.stepNumberTextColor);
            style.stepTitleTextColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_title_text_color,
                    style.stepTitleTextColor);
            style.stepSubtitleTextColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_subtitle_text_color,
                    style.stepSubtitleTextColor);
            style.nextButtonTextColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_next_button_text_color,
                    style.nextButtonTextColor);
            style.nextButtonPressedTextColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_next_button_pressed_text_color,
                    style.nextButtonPressedTextColor);
            style.lastStepCancelButtonTextColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_cancel_button_text_color,
                    style.lastStepCancelButtonTextColor);
            style.lastStepCancelButtonPressedTextColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_cancel_button_pressed_text_color,
                    style.lastStepCancelButtonPressedTextColor);
            style.errorMessageTextColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_error_message_text_color,
                    style.errorMessageTextColor);
            style.bottomNavigationBackgroundColor = vars.getColor(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_bottom_navigation_background_color,
                    style.bottomNavigationBackgroundColor);
            style.displayBottomNavigation = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_display_bottom_navigation,
                    style.displayBottomNavigation);
            style.displayStepButtons = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_display_step_buttons,
                    style.displayStepButtons);
            style.displayCancelButtonInLastStep = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_display_cancel_button_in_last_step,
                    style.displayCancelButtonInLastStep);
            style.displayNextButtonInLastStep = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_display_next_button_in_last_step,
                    style.displayNextButtonInLastStep);
            style.displayStepDataInSubtitleOfClosedSteps = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_display_step_data_in_subtitle_of_closed_steps,
                    style.displayStepDataInSubtitleOfClosedSteps);
            style.displayDifferentBackgroundColorOnDisabledElements = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_display_different_background_color_on_disabled_elements,
                    style.displayDifferentBackgroundColorOnDisabledElements);
            style.includeConfirmationStep = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_include_confirmation_step,
                    style.includeConfirmationStep);
            style.allowNonLinearNavigation = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_allow_non_linear_navigation,
                    style.allowNonLinearNavigation);
            style.allowStepOpeningOnHeaderClick = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_allow_step_opening_on_header_click,
                    style.allowStepOpeningOnHeaderClick);
            style.closeLastStepOnCompletion = vars.getBoolean(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_close_last_step_on_completion,
                    style.closeLastStepOnCompletion);
            style.alphaOfDisabledElements = vars.getFloat(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_alpha_of_disabled_elements,
                    style.alphaOfDisabledElements);
            int stepNumberFontFamilyResourceId = vars.getResourceId(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_step_number_font_family,
                    -1);
            ...
            int stepTitleFontFamilyResourceId = vars.getResourceId(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_step_title_font_family,
                    -1);
            ...
            int stepSubtitleFontFamilyResourceId = vars.getResourceId(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_step_subtitle_font_family,
                    -1);
            ...
            int stepErrorMessageFontFamilyResourceId = vars.getResourceId(
                    ernestoyaquello.com.verticalstepperform.R.styleable.VerticalStepperFormView_form_step_error_message_font_family,
                    -1);
    ...
    void initializeForm(StepperFormListener listener, ds.example.beepz.verticalstepperform.StepHelper[] stepsArray) {
    ...
    private View initializeStepHelper(int position) {
        ds.example.beepz.verticalstepperform.StepHelper stepHelper = stepHelpers.get(position);
    ...
    protected int getStepLayoutResourceId(int position, boolean isLast) {
        ...
        return ernestoyaquello.com.verticalstepperform.R.layout.step_layout;
    }

    private ds.example.beepz.verticalstepperform.StepHelper getOpenStepHelper() {
        for (int i = 0; i < stepHelpers.size(); i++) {
            ds.example.beepz.verticalstepperform.StepHelper stepHelper = stepHelpers.get(i);
            if (stepHelper.getStepInstance().isOpen()) {
                return stepHelper;
    ...
    private synchronized void openStep(int stepToOpenPosition, boolean useAnimations) {
        if (stepToOpenPosition >= 0 && stepToOpenPosition < stepHelpers.size()) {
            ...
            if (stepToClosePosition != -1) {
                ds.example.beepz.verticalstepperform.StepHelper stepToClose = stepHelpers.get(stepToClosePosition);
                stepToClose.getStepInstance().closeInternal(useAnimations);
            }

            ds.example.beepz.verticalstepperform.StepHelper stepToOpen = stepHelpers.get(stepToOpenPosition);
    ...
    private synchronized void attemptToCompleteForm(boolean isCancellation) {
        ...
        ds.example.beepz.verticalstepperform.StepHelper lastStepHelper = stepHelpers.get(stepHelpers.size() - 1);
        ds.example.beepz.verticalstepperform.Step<?> lastStep = lastStepHelper.getStepInstance();
        if (!isCancellation) {
    ...
    private void findViews() {
        formContentView = findViewById(ernestoyaquello.com.verticalstepperform.R.id.content);
        stepsScrollView = findViewById(ernestoyaquello.com.verticalstepperform.R.id.steps_scroll);
        progressBar = findViewById(ernestoyaquello.com.verticalstepperform.R.id.progress_bar);
        previousStepButton = findViewById(ernestoyaquello.com.verticalstepperform.R.id.down_previous);
        nextStepButton = findViewById(ernestoyaquello.com.verticalstepperform.R.id.down_next);
        bottomNavigationView = findViewById(ernestoyaquello.com.verticalstepperform.R.id.bottom_navigation);
    }
